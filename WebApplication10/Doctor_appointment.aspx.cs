...
namespace WebApplication10
{
    public partial class WebForm7 : System.Web.UI.Page
    {
        ...
        {
            sqlCon.Open();
            string s = " select fullname,email,mobileno,cnic from patient_basic where patient_basic.cnic in (select patient_cnic from all_appointment where all_appointment.doctor_id='"+ Session["cnic_from_login_page"] + "')";
        ...
        {
            sqlCon.Open();
            SqlCommand cmd = new SqlCommand("delete from all_appointment where patient_cnic='" + text.Text + "'", sqlCon);
            SqlCommand cmd2 = new SqlCommand("update doctor_basic set appointments_count=appointments_count-1 where cnic='"+ Session["cnic_from_login_page"] + "'", sqlCon);
